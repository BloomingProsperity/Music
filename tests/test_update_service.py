...
def test_app_version_defaults_to_v016() -> None:
    assert APP_VERSION == "0.16"
...
def test_resolve_app_version_includes_git_commit_when_available(tmp_path: pathlib.Path, monkeypatch) -> None:
    ...
    assert resolve_app_version(tmp_path) == "0.16"
...
def test_check_update_availability_reports_new_remote_revision(tmp_path: pathlib.Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("src.Infrastructure.update_service._fetch_remote_app_version", lambda: "0.16", raising=False)

    result = check_update_availability(tmp_path)

    assert result.ok is True
    assert result.update_available is True
    assert result.current_version == "0.16"
    assert result.latest_version == "0.16"
